...
function testLogLevels() {
  ...
  logger.debug('This is a DEBUG level message', { debug: 'some debug data' });
  logger.info('This is an INFO level message', { info: 'some info' });
  logger.warn('This is a WARN level message', { warning: 'be careful' });
  
  // Test error logging
  try {
    throw new Error('Test error');
  } catch (error) {
    logger.error('This is an ERROR level message', error);
  }
  
  logger.fatal('This is a FATAL level message', { error: 'critical failure' });
}

// Test logging with different data types
function testDataTypes() {
  logger.info('Testing different data types', {
    string: 'hello',
    number: 42,
    boolean: true,
    nullValue: null,
...
function testRedaction() {
  ...
  logger.info('Testing redaction of sensitive data', sensitiveData);
}
